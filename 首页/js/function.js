...
// 层级轮播    包括(透明轮播) 图片定位在同一个地方
	...
	function Zlunbo(pic,bigbannerBox,lunbodian,liactivebgColor,lunboTime,donghuaTime,lisColor,left,right,type=1){
		if(type==0){
		var flag=true;	
		const tupian=$(pic);
   ...
   //透明轮播图 
    ...
    function Tlunbo(pic,bannerBox,lis,liactivebgColor,lubotime,lisColor,zuokuohao,youkuohao){
    	const tu=$(pic);
        ...
        for(let j=0;j<li.length;j++){
        	li[j].onmouseover=function(){
        		if(flag){
				    ...
				    // banner.style.background=color[j];
				    animate(tu[now],{opacity:0},500);
				    animate(tu[j],{opacity:1},500,function(){
				    	flag=true;
				    	num=j;
				    	now=num;
				    });
...
//事件执行一次
	...
	//type: 发生什么
	//fn: 执行的程序
	//例如：one(box,"click",function(){alert(1);})
...
//下拉导航
	...
	function xialadh(btn,con){
		...
		for(let i=0;i<ul.length;i++){
			let h=parseInt(attr(ul[i],"height")); //获取ul[i]的高度
